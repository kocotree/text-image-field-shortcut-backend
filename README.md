...
`MEMORY_TRIM_AFTER_IMAGE_REQUEST` 是内存回收开关，默认关闭。启用后，服务会在图片响应发送完成时在 Linux 容器内调用 `malloc_trim(0)` 归还 glibc 保留的空闲堆页。日志 `memory.image_request.release.completed` 包含回收前后 RSS；该操作可能短暂阻塞进程。
