...
- `FEISHU_ALERT_ENABLED`（默认 `false`）
- `FEISHU_ALERT_WEBHOOK_URL`
- `FEISHU_ALERT_SECRET`
- `SERVICE_NAME`（默认 `text-image-field-shortcut-backend`）
- `APP_ENV`（默认 `production`）
- `FALLBACK_ALERT_WINDOW_SECONDS`（默认 `300`）
- `FALLBACK_ALERT_THRESHOLD`（默认 `3`）
- `FALLBACK_ALERT_COOLDOWN_SECONDS`（默认 `900`）
- `CRITICAL_ALERT_COOLDOWN_SECONDS`（默认 `60`）
- `PRIMARY_RECOVERY_SUCCESS_THRESHOLD`（默认 `3`）
- `ALERT_INCIDENT_TTL_SECONDS`（默认 `86400`）
- `FEISHU_CONNECT_TIMEOUT_SECONDS`（默认 `2`）
- `FEISHU_READ_TIMEOUT_SECONDS`（默认 `3`）
- `FEISHU_POOL_TIMEOUT_SECONDS`（默认 `1`）
